"""Store Summary v2 — summaries.sqlite keyed by content_hash (copy mindmap/store.py)."""
...
def db_path() -> Path:
    override = (os.environ.get("SUMMARIES_DB_PATH") or "").strip()
    if override:
        return Path(override)
    memory_dir = (os.environ.get("MEMORY_DIR") or "").strip()
    if memory_dir:
        return Path(memory_dir) / "summaries.sqlite"
    from shared.paths import BE_ROOT
    return BE_ROOT / "memory" / "summaries.sqlite"
...
def _ensure_columns(conn: sqlite3.Connection) -> None:
    ...
    if "user_id" not in cols:
        conn.execute("ALTER TABLE summaries ADD COLUMN user_id TEXT")
...
def init_db() -> None:
    with _lock:
        ...
        try:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_sum_hash ON summaries(content_hash)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_sum_user ON summaries(user_id, content_hash)")
            conn.commit()
        finally:
            conn.close()
...
def save_record(record: dict, user_id: Optional[str] = None) -> None:
    ...
    with _lock:
        ...
        try:
            conn.execute(
                """
                INSERT OR REPLACE INTO summaries(id, content_hash, sources_json, created_at, record_json, user_id)
                VALUES(?,?,?,?,?,?)
                """,
                (
...
def delete_by_source(stem: str) -> int:
    ...
    with _lock:
        conn = get_conn()
        try:
            cur = conn.execute("SELECT id, sources_json FROM summaries")
            ids = [row[0] for row in cur.fetchall() if target in _decode_sources(row[1])]
            deleted = 0
            for summary_id in ids:
                res = conn.execute("DELETE FROM summaries WHERE id=?", (summary_id,))
...
def migrate_from_json(json_path: Path) -> int:
    """Import 1 lần summaries.json cũ (shape {id,title,data:{summary,...},sources,createdAt})
    → record schema_version 1 với body ở summary_md; xong rename .migrated (chặn hồi sinh)."""
    init_db()
    p = Path(json_path)
    if not p.exists():
        return 0
    ...
    with _lock:
        ...
        try:
            for item in payload:
                ...
                if "summary_md" not in record:
                    record["summary_md"] = str(data.get("summary") or record.get("summary") or "")
                normalized = _normalized_record(record)
                cur = conn.execute(
                    """
                    INSERT OR IGNORE INTO summaries(id, content_hash, sources_json, created_at, record_json)
                    VALUES(?,?,?,?,?)
                    """,
                    (
                        str(normalized.get("id") or ""),
